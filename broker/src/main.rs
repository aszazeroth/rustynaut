...
mod tests {
    ...
    fn test_accept_offer_creates_active_transfer() {
        ...
        shared.register_offer(sender, "alice", "lobby", "report.pdf", 12345);

        assert_eq!(
            shared.accept_offer(acceptor, "lobby", "alice", "report.pdf"),
            Some((1, sender))
        );
        assert_eq!(shared.file_transfers.pending_offer_count(), 0);
        assert_eq!(shared.file_transfers.next_transfer_id(), 1);

        let transfer = shared.get_transfer(1).unwrap();
    ...
    fn test_accept_offer_adds_multiple_acceptors_to_existing_offered_transfer() {
        ...
        assert_eq!(
            shared.accept_offer(first_acceptor, "lobby", "alice", "report.pdf"),
            Some((1, sender))
        );
        assert_eq!(
            shared.accept_offer(second_acceptor, "lobby", "alice", "report.pdf"),
            Some((1, sender))
        );
        assert_eq!(shared.file_transfers.next_transfer_id(), 1);
    ...
    fn test_accept_offer_rejects_when_transfer_already_transferring() {
        ...
        shared.register_offer(sender, "alice", "lobby", "report.pdf", 12345);
        let (transfer_id, _) = shared
            .accept_offer(first_acceptor, "lobby", "alice", "report.pdf")
            .unwrap();
        shared.get_transfer_mut(transfer_id).unwrap().state = TransferState::Transferring;
        ...
        let transfer = shared.get_transfer(transfer_id).unwrap();
        assert_eq!(transfer.acceptors.len(), 1);
        assert!(transfer.acceptors.contains(&first_acceptor));
        assert!(!transfer.acceptors.contains(&second_acceptor));
    }
    ...
    fn test_find_latest_offer_filters_by_room_and_sender_and_returns_newest() {
        ...
        shared.register_offer(addr(10_031), "alice", "lobby", "old.pdf", 1);
        shared.register_offer(addr(10_032), "alice", "dev", "other-room.pdf", 2);
        shared.register_offer(addr(10_033), "bob", "lobby", "other-user.pdf", 3);
        shared.register_offer(addr(10_034), "alice", "lobby", "new.pdf", 4);

        assert_eq!(
            shared.find_latest_offer("lobby", "alice"),
            Some("new.pdf".to_string())
        );
        assert_eq!(
            shared.find_latest_offer("dev", "alice"),
            Some("other-room.pdf".to_string())
        );
        assert_eq!(shared.find_latest_offer("missing", "alice"), None);
        assert_eq!(shared.find_latest_offer("lobby", "carol"), None);
    }
    ...
    fn test_list_offers_filters_to_current_room() {
        ...
        shared.register_offer(addr(10_041), "alice", "lobby", "report.pdf", 123);
        shared.register_offer(addr(10_042), "bob", "lobby", "notes.txt", 456);
        shared.register_offer(addr(10_043), "alice", "dev", "secret.pdf", 789);

        let mut offers = shared.list_offers("lobby");
